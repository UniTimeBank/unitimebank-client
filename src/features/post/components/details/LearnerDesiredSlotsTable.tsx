...
interface LearnerDesiredSlotsTableProps {
  slots?: TimeSlot[];
}

export const LearnerDesiredSlotsTable: React.FC<LearnerDesiredSlotsTableProps> = ({ slots = [] }) => {
  return (
    <div className="bg-white rounded-3xl p-6 border border-gray-200/80 shadow-xs space-y-4">
      <div className="flex items-center gap-2 border-b border-gray-100 pb-3">
        <Calendar className="w-5 h-5 text-primary-500" />
        <h3 className="text-base font-black text-gray-900">Khung Giờ Rảnh Mong Muốn Học</h3>
      </div>

      {slots.length === 0 ? (
        <div className="p-4 rounded-2xl bg-gray-50 text-center text-xs font-semibold text-gray-500">
          Học viên sẵn sàng linh hoạt thu xếp giờ học theo thỏa thuận với Mentor.
        </div>
      ) : (
